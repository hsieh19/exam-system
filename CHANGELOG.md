...
- **飞书免登与免确认**:
  ...
  - 修复了飞书 JSSDK 官方链接 ORB 阻断问题，确保 SDK 稳定加载。
- **账号单点登录 (SSO) 限制**:
  - **单设备强制下线**: 引入单账号单活动会话限制，防止考生通过代考或多设备共享账号。新设备登录将自动踢掉旧设备。
  - **实时下线通知**: 集成 SSE (Server-Sent Events) 技术，当账号在别处登录时，原设备将立即收到通知并强制跳转至登录页。
  - **会话状态感知**: 前端 `authFetch` 增强，能够精准捕获 401 状态并提示“账号已在其他设备登录”。

### 🛠️ 兼容性与性能优化
- **JSSDK 按需加载**: 
  - 彻底解决了在非飞书环境下加载 SDK 导致的 `cannot find pc bridge` 错误。
  - 采用动态按需加载策略，仅在飞书环境或用户点击飞书登录时才加载相关资源。
- **会话管理升级**: 
  - 重构了 `SessionStore` (Memory/Redis)，新增 `userTokens` 映射表，支持高效的“用户-令牌”对齐检查。
  - 增强了身份验证中间件，支持在每次请求时验证令牌的时效性与唯一性。

...
- **新增试卷归档机制**：管理端主列表新增“归档”操作，可以将已考完或过期的试卷一键归档。归档试卷将对学生端（待考试卷和排行榜）完全隐藏。
- **归档管理与状态追溯**：管理后台新增“归档管理”浮层，支持查看已归档的试卷，并提供“取消归档”与“彻底删除”的功能。
- **考试分析与得分排行过滤**：管理端“考试分析”下拉列表与“得分及排行榜”页面下拉菜单均同步过滤隐藏已归档的试卷，支持“取消归档”后再行分析与成绩查看，保持管理后台的数据和页面整洁。

### 🚑 判断题导入及判分错误修复

- **解析逻辑修正**：纠正了题库 Excel 导入时判断题正确答案被错误存入为 `"true"` / `"false"` 的问题，正确将其映射为标准选项 `"A"` / `"B"`。
- **导出兼容性优化**：导出试卷为 Excel 时对历史旧数据 `"true"` / `"false"` 和新数据 `"A"` / `"B"` 做了安全兼容，避免导出格式不一致。
- **历史数据无缝订正**：提供跨平台自适应的数据库订正脚本，一键修复存量脏数据，保障考生得分与题目分析的绝对准确。

## [1.6.1] - 2026-04-28

### 📊 排行榜访问修复

- **修复排行榜详情 403 错误**：解决了考生在选择已参加过的试卷后，因部门变动被后端拦截导致无法显示排名的问题。现在，只要有考试记录，即可合法查看对应试卷的排行榜数据。

## [1.6.0] - 2026-04-28

### 🚀 飞书多分组支持全面达成

- **排行榜可见性兜底逻辑**：新增“参加过即显示”机制。即使考生所属分组发生变动，只要其曾参加过某场考试并存有记录，仍可在排行榜中查看该试卷及其排名。
- **全链路权限校验重构**：彻底清理了进入考试、提交成绩、管理试卷等核心流程中残余的单分组精准匹配代码，全面切换为 `hasCommonGroup` 交集校验。
- **系统稳定性增强**：修复了多分组环境下可能导致的“无权参加考试”或“无权提交成绩”的潜在 Bug。

## [1.5.9] - 2026-04-28

### 📊 排行榜多分组适配修复

- **考生排行榜可见性修复**：修复了多分组考生无法在“得分及排行榜”中看到被分配试卷的问题。
- **完善排行榜权限校验**：全面应用 `hasCommonGroup` 逻辑，确保多分组用户在查看排行榜时的权限被正确识别。
- **应考人数统计优化**：修正了排行榜中“应考人数”的计算逻辑，使其能够正确识别并统计拥有多个分组身份的考生。

## [1.5.8] - 2026-04-28

### 🔐 组管管理权限修复

- **跨创建者管理支持**：修复了组管无法看到或管理（编辑、推送、删除）由他人创建但属于其管辖分组的试卷的问题。
- **完善 UI 权限判断**：管理后台现在会根据试卷的 `groupId` 自动为管辖该分组的管理员开启操作权限。
- **优化推送逻辑**：支持多分组管理员在推送试卷时，默认覆盖其管辖的所有分组。

## [1.5.7] - 2026-04-28

### 🔐 飞书多分组权限深度适配

- **支持多分组权限校验**：重构权限系统，支持飞书用户所属的多个分组（逗号分隔字符串）与系统资源的正确匹配。
- **管理权限下放**：拥有多个分组权限的管理员现在可以统一管理其管辖范围内的所有用户、题目、试卷及排行榜。
- **修复排行榜与试卷列表可见性**：彻底解决了 Feishu 多分组场景下，由于字符串精准匹配失效导致的“选择不到试卷”及“无法查看排行榜”的问题。

### 🛠️ 数据库适配器优化

- **增强过滤能力**：更新 `getUsers`、`getQuestions` 和 `getPapers` 接口，支持通过多个分组 ID 进行联合查询。
- **完善权限校验辅助函数**：引入 `hasCommonGroup` 逻辑，统一前后端的分组交叉验证标准。

## [1.5.6] - 2026-03-19

### 🚀 系统日志与题库审计增强

- **修复日志筛选异常**：修复系统日志界面“全部对象”与“全部操作”下拉框为空且无法正常筛选的 Bug。
- **完善题目更新日志**：题目变动记录全面中文化，并动态解析专业、设备类型名称，提供精细化的修改内容描述（如：类型、专业、内容等）。
- **题库最后修改人**：在题库列表中新增“最后修改人”展示，并实现跨端后端的自动记录与同步功能。

### 🧹 数据库与性能调优

- **日志归档策略**：实现系统日志按年（365天）自动归档策略，确保日志表长期运行不会影响系统性能。
- **查询索引增强**：为统计分析和排行榜查询的关键路径配齐数据库索引，大幅降低大数据环境下的查表延迟。

### 🎨 推送记录界面优化

- **布局垂直对齐**：重构推送记录弹窗，将考试开始与截止时间分为两行显示，并与推送信息保持垂直轴线对齐。
- **列宽科学调节**：大幅压缩固定内容的列宽（时间列），将 50% 以上的剩余空间智能分配给长文本的目标分组与用户列。

## [1.5.5] - 2026-02-26

### 🧹 代码重构与性能优化

- **后端公共模块提取**：新增 `src/utils/exam-helpers.js`，将考试记录、提交成绩、正确率分析中重复的哈希计算、试卷规则解析、选项打乱、答案标准化等核心逻辑统一封装，累计清理重复代码约 200 余行。
- **前端逻辑收敛**：
  - 提取公共函数 `renderMajorDropdown` 和 `renderDeviceDropdown`，统一管理导入导出、题库筛选、选题器中的级联下拉逻辑。
  - 修复前端 `admin.js` 中 `initSelectorMustFilterDropdown` 函数定义冲突的 Bug。

### 🎨 界面与交互优化

- **题库筛选顺序调整**：按照逻辑自然顺序（归属 -> 专业 -> 设备 -> 题型 -> 题目）重新排列题库列表上方的筛选按钮，提升操作直觉。
- **手动选题器视觉降噪**：删除手动选题器中下拉框上方的标题文本（如“题库/分组”等），改用垂直居中布局，使弹出界面更加简洁紧凑。
